...
public class PlanManageGatewayController {

    private static final Logger logger = LoggerFactory.getLogger(PlanManageGatewayController.class);
    @Value("${server.rehost}")
    private String rehost;//在application.yml中配置的地址参数

    @PostMapping("/new")
    public JSONObject createPlanManage(@RequestBody JSONObject params){
        logger.info(params.toString());//日志打印
        params.put("title","planManage"); //title表示collectionName
    ...
    @GetMapping("/{id}")
    public JSONObject getPlanManageById(@PathVariable String id){
        RestTemplate restTemplate = new RestTemplate();
    ...
    public JSONObject updateDocumentById(@PathVariable String id
        ...
        pathParam.put("id", id);
        pathParam.put("collectionName","planManage"); //只需考虑这行，将collectionName更改即可
